        ...
        <div class="upsell-product home2">
            <div class="container">
                ...
                <div class="row">
                    ...
                    WHERE tbl_order.id = tbl_order_detail.orderid 
                    AND STATUS = 'Thành công' 
                    GROUP BY productid
                    LIMIT 8";
                        $run = mysqli_query($conn, $query);
                        while ($listtopsellproduct = mysqli_fetch_array($run)) {
                            $query = 'SELECT * FROM `tbl_product` WHERE id =' . $listtopsellproduct['productid'];
                            $run1 = mysqli_query($conn, $query);
                            $topsellproduct = mysqli_fetch_array($run1);
                            ?>
                            <div class="col-md-12">
                                <div class="single-product">
                                    <div class="product-img">
                                        <a href="single-product.html">
                                            <a href="?page=product-detail&productid=<?php echo $topsellproduct['id'] ?>">

                                                <?php $query = 'SELECT * FROM `tbl_img_product` WHERE id_product =' . $topsellproduct['id'] . '  limit 9';
                                                    $run2 = mysqli_query($conn, $query);
                                                    if (mysqli_num_rows($run2) == 9) {
                                                        ?>
                                                    <img src="images/product/<?php echo $topsellproduct['image'] ?>" alt="" class="primary-img-1">
                                                    <?php
                                                            while ($secondimage = mysqli_fetch_array($run2)) { ?>
                                                        <img src="images/product/<?php echo $secondimage['file_name'] ?>" alt="" class="primary-img-2">
                                                    <?php }
                                                        } else { ?>
                                                    <img src="images/product/<?php echo $topsellproduct['image'] ?>" alt="">
                                                <?php } ?>
                                            </a>
                                        </a>
                                    </div>
                                    ...
                                    <div class="product-price">
                                        <div class="product-name">
                                            <a href="single-product.html" title="Fusce aliquam"><?php echo $topsellproduct['name'] ?></a>
                                        </div>
                                        <div class="price-rating">
                                            <span><?php echo $topsellproduct['sale'] ?></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="container">
                ...
                <div class="row">
                    <div class="related-slider">
                        ...
                        $query = 'SELECT * FROM `tbl_product` WHERE category=' . $product['category'] . ' ORDER BY id DESC limit 8 ';
                        $run = mysqli_query($conn, $query);
                        while ($relatedproduct = mysqli_fetch_array($run)) {

                            ?>
                            <div class="col-md-12">
                                <div class="single-product">
                                    <div class="product-img">
                                        <a href="?page=product-detail&productid=<?php echo $relatedproduct['id'] ?>">

                                            <?php $query = 'SELECT * FROM `tbl_img_product` WHERE id_product =' . $relatedproduct['id'] . '  limit 9';
                                                $run1 = mysqli_query($conn, $query);
                                                if (mysqli_num_rows($run1) == 9) {
                                                    ?>
                                                <img src="images/product/<?php echo $relatedproduct['image'] ?>" alt="" class="primary-img-1">
                                                <?php
                                                        while ($secondimage = mysqli_fetch_array($run1)) { ?>
                                                    <img src="images/product/<?php echo $secondimage['file_name'] ?>" alt="" class="primary-img-2">
                                                <?php }
                                                    } else { ?>
                                                <img src="images/product/<?php echo $relatedproduct['image'] ?>" alt="">
                                            <?php } ?>
                                        </a>
                                    </div>
                                    <div class="product-price">
